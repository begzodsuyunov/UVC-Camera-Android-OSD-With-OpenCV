...
//    time_t c_start, c_end;
    ...
    int previewFrameCount1 = 0;
    int previewFrameCount2 = 0;
    double previewStartTime = 0.0;
    double previewEndTime = 0.0;
    double previewFPS = 0.0;
    double previewStartTime1 = 0.0;
    double previewEndTime1 = 0.0;
    double previewFPS1 = 0.0;
...
//                    c_start = clock();
                    ...
                    cv::Mat frameMat(frame->height, frame->width, CV_8UC4, frame->data);
                    ...
                    cv::Scalar textColor(255, 255, 255); // Y=255, U=128, V=128
...
//                    // Add additional text next to the time text
                    ...
                    previewEndTime1 = cv::getTickCount() / cv::getTickFrequency();
                    double elapsedTime1 = previewEndTime1 - previewStartTime1;
                    if (elapsedTime1 >= 1.0) {
                        previewFPS1 = previewFrameCount2 / elapsedTime1;
                        LOGI("Preview FPS2: %.2f", previewFPS1);

                        previewFrameCount2 = 0;
                        previewStartTime1 = previewEndTime1;
                    }
...
//                    LOGI("uvc_mjpeg2yuyv time: %f", (double) (c_end - c_start) / CLOCKS_PER_SEC);
                    ...
                    if (LIKELY(!result)) {
//                        c_start = clock();
//                        frame = draw_preview_one(frame, &mPreviewWindow,
//                                                 PREVIEW_PIXEL_BYTES);
...
//                        LOGI("uvc_any2rgbx time: %f", (double) (c_end - c_start) / CLOCKS_PER_SEC);
                    ...
                    previewEndTime = cv::getTickCount() / cv::getTickFrequency();
                    double elapsedTime = previewEndTime - previewStartTime;

                    if (elapsedTime >= 1.0) {
                        previewFPS = previewFrameCount / elapsedTime;
                        LOGI("Preview FPS: %.2f", previewFPS);
...
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, int pixelBytes) {
    int b = 0;
    pthread_mutex_lock(&preview_mutex);
    {
        b = *window != NULL;
